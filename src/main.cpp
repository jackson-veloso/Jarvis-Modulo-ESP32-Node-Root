...
// Prototype
void getMessageSensors(JsonDocument doc);
...
void getMessageSensors(JsonDocument doc)
{
  String idSensor = doc["idSensor"];
  sensores[idSensor] = doc;

  String json;
  serializeJson(sensores, json);

  Serial.print("SENSORES RECEBIDOS = ");
  Serial.println(json);
}
...
void webServerEnable()
{
  ...
  server.on("/getModules", HTTP_GET, [](AsyncWebServerRequest *request)
            {
              String json;
